...
export default function TripleScreenRoom({ onExit, isHost = false, roomId }: Props) {
        ...
        <div className="flex flex-col items-center justify-center h-full">
          {isHost ? (
            ...
            <div className="flex flex-col items-center animate-pulse">
              ...
              <p className="text-slate-500 font-mono tracking-widest uppercase text-sm">{t('waitingForHost')}</p>
            </div>
          )}
        </div>
      );
  ...
  return (
    <div className="fixed inset-0 z-50 bg-[#0A0E14] flex flex-col overflow-hidden font-sans" dir={dir}>
      {/* Top Bar */}
      <div className="absolute top-0 left-0 right-0 h-20 bg-gradient-to-b from-black/80 to-transparent z-40 flex items-center justify-between px-6 pointer-events-none">
        <div className="flex items-center gap-3">
          <div className="w-3 h-3 rounded-full bg-red-500 animate-pulse shadow-[0_0_10px_rgba(239,68,68,0.8)]" />
          <span className="text-white font-mono font-bold tracking-widest text-sm opacity-80 uppercase">{t('room')}: {roomId}</span>
        ...
        <button 
          onClick={onExit}
          className="pointer-events-auto flex items-center gap-2 px-4 py-2 bg-slate-800/80 hover:bg-red-500/80 border border-slate-700 hover:border-red-500 rounded-full text-white transition-all duration-300 backdrop-blur-md shadow-lg"
        >
          <LogOut className="w-4 h-4" />
          <span className="text-sm font-bold">{t('exitRoom') || 'Exit'}</span>
        </button>
      </div>
      ...
      >
        ...
        >
          {slots.map((slot, index) => (
            <div key={index} className="w-1/3 h-full p-4 md:p-6 pb-2">
              <div className="w-full h-full bg-[#0F1520] md:rounded-[2rem] rounded-3xl border-2 border-slate-800/80 shadow-[0_10px_40px_rgba(0,0,0,0.8)] overflow-hidden relative">
      ...
      <div className="absolute bottom-0 left-0 right-0 h-[100px] bg-[#0B1120]/90 backdrop-blur-lg border-t border-slate-800 flex items-center px-6 overflow-x-auto gap-4 z-40 shadow-[0_-10px_30px_rgba(0,0,0,0.5)] scrollbar-hide">
        <div className="sticky left-0 bg-[#0B1120]/90 pr-4 py-2 flex flex-col justify-center border-r border-slate-800 mr-2 z-10 hidden md:flex">
          <span className="text-slate-400 text-xs font-bold uppercase tracking-widest block mb-1">{t('room')}</span>
          <div className="text-emerald-400 font-bold text-sm flex items-center gap-2">
            <UsersIcon /> {visitors.length + 1} {t('online')}
          </div>
        </div>

        {/* ALWAYS show my own local camera feed at the bottom for an active feeling, even if host is broadcasting above */}
        <div className="relative w-16 h-16 rounded-2xl overflow-hidden border-2 border-emerald-500 flex-shrink-0 shadow-lg shadow-emerald-500/20 group">
          <LiveVideo stream={myStream} muted={true} />
          <div className="absolute bottom-0 left-0 right-0 bg-black/60 backdrop-blur-sm text-[10px] text-white text-center font-bold tracking-wider px-1 py-0.5">{t('you')}</div>
        </div>

        {/* Display all connected visitors */}
        {visitors.map((v, i) => (
          <div key={i} className="flex flex-col items-center w-16 flex-shrink-0 gap-1 animate-in zoom-in duration-300">
             <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-slate-700 to-slate-800 border border-slate-600 flex items-center justify-center text-white font-bold text-xl shadow-lg relative">
                 {v.name ? v.name.charAt(0).toUpperCase() : 'V'}
                 <div className="absolute -bottom-1 -right-1 w-4 h-4 bg-emerald-500 border-2 border-[#0B1120] rounded-full"></div>
             </div>
             <span className="text-[10px] text-slate-300 truncate w-full text-center mt-1">{v.name}</span>
          </div>
        ))}
        
        {visitors.length === 0 && (
          <div className="flex-1 flex items-center justify-center text-slate-500 text-sm font-mono tracking-wide">
            {isHost ? t('waitingForSomeone') : t('onlyVisitor')}
          </div>
        )}
      </div>
    </div>
