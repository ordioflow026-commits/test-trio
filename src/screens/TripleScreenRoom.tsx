  ...
  const renderSlotContent = (slot: SlotData, index: number) => {
    ...
    return (
      <div className="flex flex-col items-center justify-center h-full w-full relative group">
        ...
        {slot.type === 'youtube' && <div className="w-full h-full bg-black flex items-center justify-center">{slot.url ? <SyncYouTubePlayer videoId={slot.url} isHost={isHost} roomId={roomId as string} /> : <Youtube className="w-20 h-20 text-red-500/50" />}</div>}
