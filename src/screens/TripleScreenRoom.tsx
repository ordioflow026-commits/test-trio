  ...
  const renderSlotContent = (slot: SlotData, index: number) => {
    ...
    return (
      <div className="flex flex-col items-center justify-center h-full w-full relative group">
        ...
        {slot.type === 'media' && <div className="w-full h-full bg-black relative"><SyncMediaViewer url={slot.url} canInteract={editable} onUploadSuccess={(newUrl) => updateSlot(index, { type: 'media', url: newUrl })} /></div>}
