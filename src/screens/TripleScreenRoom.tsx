...
export default function TripleScreenRoom({ onExit, isHost = false, roomId, roomName, onNameSync }: Props) {
  ...
  const renderSlotContent = (slot: SlotData, index: number) => {
    ...
    if (slot.type === 'menu') return (
        <div className="flex flex-col items-center justify-start h-full w-full max-w-5xl mx-auto p-4 overflow-y-auto relative bg-[#0A0E14]" dir={dir}>
          <div className="flex justify-center items-center w-full mb-8 pt-10 relative">
            ...
            <button onClick={() => updateSlot(index, { type: 'empty' })} className={`absolute ${dir === 'rtl' ? 'left-4' : 'right-4'} top-0 p-4 bg-slate-800 rounded-2xl text-slate-400`}><X className="w-6 h-6" /></button>
          </div>
