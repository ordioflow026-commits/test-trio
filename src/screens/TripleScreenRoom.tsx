  ...
  const renderSlotContent = (slot: SlotData, index: number) => {
    ...
    return (
      <div className="flex flex-col items-center justify-center h-full w-full relative group">
        ...
        {slot.type === 'media' && <div className="w-full h-full bg-black relative overflow-hidden"><SyncMediaViewer url={slot.url} canInteract={canInteractInside} isLocalOnly={!editable} onUploadSuccess={(newUrl) => updateSlot(index, { type: 'media', url: newUrl })} roomId={roomId} isHost={isHost} slotIndex={index} viewMode={viewMode} /></div>}
        {slot.type === 'notes' && (
