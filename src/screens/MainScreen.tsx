...
export default function MainScreen() {
  ...
  useEffect(() => {
    const channel = supabase.channel('global_live_streams_notif')
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'live_streams' }, (payload) => {
        const followed = JSON.parse(localStorage.getItem('trio_followed_hosts') || '[]');
        if (followed.includes(payload.new.host_id) && payload.new.host_id !== user?.id) {
          const newNotif = {
            id: 'notif_' + Date.now(),
            text: dir === 'rtl' ? `${payload.new.host_name} بدأ بثاً مباشراً: ${payload.new.topic}` : `${payload.new.host_name} is live: ${payload.new.topic}`,
            read: false,
                  ...
                  <div
                    ...
                    onClick={() => {
                      if (n.action === 'open_stream') {
                        ...
                        setTimeout(() => window.dispatchEvent(new CustomEvent('open-live-stream', { detail: n.streamId })), 300);
                      }
                    }}
