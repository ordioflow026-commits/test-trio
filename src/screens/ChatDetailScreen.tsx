...
import { ArrowLeft, Phone, Video, Mic, Paperclip, Camera, Send, Image as ImageIcon, FileText, File, Check, Copy, Trash2, X, Square, ArrowDown, CornerUpLeft } from 'lucide-react';
...
export default function ChatDetailScreen() {
  ...
  const logCallInChat = async (isVideo: boolean) => {
    ...
    const tempMsg: Message = { id: tempId, sender_id: user.id, receiver_id: contactProfileId, content, created_at: new Date().toISOString(), status: 'sending' };
    
    setMessages(prev => [...prev, tempMsg]);
    setTimeout(scrollToBottom, 50);

    const { error } = await supabase.from('messages').insert({ id: tempId, sender_id: user.id, receiver_id: contactProfileId, content, status: 'sent' });
  ...
  const handleSendMessage = async () => {
    ...
    if (replyToMsg) {
      const senderName = replyToMsg.sender_id === user.id ? 'أنت' : contact.name;
      let shortContent = replyToMsg.content;
      
      if (shortContent.startsWith('Audio: ')) shortContent = '🎵 رسالة صوتية';
      else if (shortContent.startsWith('File: ')) shortContent = '📎 ملف مرفق';
      else if (shortContent.match(/^\[REPLY\|(.*?)\|(.*?)\] ([\s\S]*)$/)) {
        const match = shortContent.match(/^\[REPLY\|(.*?)\|(.*?)\] ([\s\S]*)$/);
        shortContent = match ? match[3] : shortContent;
      }
      
      if (shortContent.length > 50) shortContent = shortContent.substring(0, 50) + '...';
      finalContent = `[REPLY|${senderName}|${shortContent}] ${finalContent}`;
      setReplyToMsg(null);
    }
  ...
  const handleCopyMultiple = async () => {
    const textsToCopy = messages
      ...
      .map(m => {
        const actualContent = m.content.match(/^\[REPLY\|(.*?)\|(.*?)\] ([\s\S]*)$/) ? m.content.match(/^\[REPLY\|(.*?)\|(.*?)\] ([\s\S]*)$/)![3] : m.content;
        if (actualContent.startsWith('Audio: ')) return '[رسالة صوتية]';
        if (actualContent.startsWith('File: ')) return '[ملف مرفق]';
  ...
  const handleDeleteForEveryoneMultiple = async () => {
    ...
    setTimeout(async () => {
      ...
      setSelectedMessageIds([]);
      setMessages(prev => prev.filter(m => !idsToDelete.includes(m.id)));
      for (const id of idsToDelete) await supabase.from('messages').delete().eq('id', id);
    }, 50);
  };
  ...
  const sendRecordedAudio = async () => {
    ...
    try {
      ...
      setRecordedAudioBlob(null);
    } catch (err) {} finally { setUploadingCount(prev => Math.max(0, prev - 1)); }
  };
  ...
  return (
    <div className="flex flex-col h-screen bg-[#0f172a] font-sans relative overflow-hidden" dir={dir}>
      ...
      {selectedMessageIds.length > 0 ? (
        <header className="flex items-center justify-between px-4 py-4 z-30 bg-[#0f283d] text-white shadow-md border-b border-white/10 transition-all">
          <div className="flex items-center gap-4">
            <button onClick={() => setSelectedMessageIds([])} className="p-2 -ml-2 hover:bg-white/10 rounded-full transition-colors"><X className="w-6 h-6 text-white" /></button>
          ...
          <div className="flex items-center gap-2">
            ...
            {selectedMessageIds.length === 1 && (
              <button onClick={() => {
                setReplyToMsg(messages.find(m => m.id === selectedMessageIds[0]) || null);
                setSelectedMessageIds([]);
              }} className="p-2 hover:bg-white/10 rounded-full transition-colors">
                <CornerUpLeft className="w-6 h-6 text-white" />
              </button>
            )}
            <button onClick={handleCopyMultiple} className="p-2 hover:bg-white/10 rounded-full transition-colors"><Copy className="w-6 h-6 text-white" /></button>
            <button onClick={() => setShowDeleteOptions(true)} className="p-2 hover:bg-white/10 rounded-full transition-colors"><Trash2 className="w-6 h-6 text-white" /></button>
          </div>
        </header>
      ) : (
        <header className="flex items-center justify-between px-3 py-4 z-20 text-white transition-all">
          ...
          <div className="flex items-center gap-4 pr-2">
            <button onClick={() => { if (!zp || !contactProfileId) return; const targetZegoId = contactProfileId.replace(/[^a-zA-Z0-9]/g, '').substring(0, 16); zp.sendCallInvitation({ callees: [{ userID: targetZegoId, userName: contact.name }], callType: 1, timeout: 60 }).catch(console.error); logCallInChat(true); }} className="p-2 hover:bg-white/10 rounded-full transition-colors"><Video strokeWidth={1.5} className="w-[22px] h-[22px] text-white" /></button>
            <button onClick={() => { if (!zp || !contactProfileId) return; const targetZegoId = contactProfileId.replace(/[^a-zA-Z0-9]/g, '').substring(0, 16); zp.sendCallInvitation({ callees: [{ userID: targetZegoId, userName: contact.name }], callType: 0, timeout: 60 }).catch(console.error); logCallInChat(false); }} className="p-2 hover:bg-white/10 rounded-full transition-colors"><Phone strokeWidth={1.5} className="w-[20px] h-[20px] text-white" /></button>
          </div>
        </header>
      )}

      <main ref={mainScrollRef} onScroll={handleScroll} className="flex-1 overflow-y-auto w-full relative z-10 px-4 flex flex-col gap-2 py-4 scroll-smooth">
         {isLoading ? (
            ...
            <>
              ...
              {messages.map((msg) => {
                ...
                return (
                  <React.Fragment key={msg.id}>
                    {showDateSeparator && (
                      <div className="flex justify-center my-4 animate-in fade-in duration-500"><span className="px-4 py-1 bg-slate-800/60 text-slate-200 text-[11px] font-medium rounded-full shadow-sm backdrop-blur-md border border-slate-700/50">{formatDateSeparator(msg.created_at)}</span></div>
                    )}
                    <div className={`flex items-center w-full py-1.5 px-2 rounded-xl transition-colors animate-in fade-in duration-300 ${isSelected ? 'bg-blue-500/20' : ''}`} onClick={() => { if (selectedMessageIds.length > 0) setSelectedMessageIds(prev => prev.includes(msg.id) ? prev.filter(id => id !== msg.id) : [...prev, msg.id]); }}>
                      {selectedMessageIds.length > 0 && (
                        <div className="mx-2 shrink-0 w-6 h-6 rounded-full border-2 flex items-center justify-center transition-colors" style={{ borderColor: isSelected ? '#3b82f6' : 'rgba(255,255,255,0.4)', backgroundColor: isSelected ? '#3b82f6' : 'transparent' }}>
                      ...
                      <div className={`flex w-full ${isMe ? 'justify-end' : 'justify-start'}`}>
                        <div onDoubleClick={() => handleReaction(msg)} onContextMenu={(e) => { e.preventDefault(); if (!selectedMessageIds.includes(msg.id)) setSelectedMessageIds(prev => [...prev, msg.id]); }} className={`max-w-[85%] sm:max-w-[75%] px-4 py-2.5 rounded-2xl shadow-sm text-white select-none relative ${isMe ? 'bg-[#00b4d8] rounded-br-none' : 'bg-slate-800/80 rounded-bl-none border border-slate-700/50'}`}>
                          ...
                          {isReply && (
                            <div className="bg-black/20 border-l-4 border-[#00E5FF] p-2 rounded-lg mb-2 opacity-90 shadow-sm cursor-pointer hover:bg-black/30 transition-colors">
                              <p className="text-[#00E5FF] text-[11px] font-bold mb-0.5">{replySender}</p>
                              <p className="text-slate-200 text-xs line-clamp-2 max-w-[220px]">{replyText}</p>
                            </div>
                          )}
              ...
              {uploadingCount > 0 && <div className="flex justify-end mb-2"><div className="max-w-[75%] px-4 py-3 rounded-2xl bg-[#00b4d8] text-white rounded-br-sm shadow-sm flex items-center gap-3"><div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin"></div><span className="text-sm">جاري الإرسال...</span></div></div>}
      ...
      {showScrollButton && (
        <button onClick={scrollToBottom} className="absolute bottom-24 right-4 z-30 p-3 bg-[#0f283d] text-white rounded-full shadow-2xl border border-white/10 hover:bg-[#1a3a54] transition-all animate-in zoom-in duration-200"><ArrowDown className="w-5 h-5" /></button>
      )}
      ...
      {showDeleteOptions && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => setShowDeleteOptions(false)}>
          <div className="bg-slate-800 rounded-2xl w-full max-w-xs overflow-hidden border border-slate-700 shadow-2xl animate-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
            <button onClick={handleDeleteForMeMultiple} className="w-full p-5 text-start text-red-400 flex items-center gap-3 hover:bg-slate-700 transition-colors"><Trash2 className="w-5 h-5" /> <span className="font-medium">حذف لدي</span></button>
            {selectedMessageIds.every(id => messages.find(m => m.id === id)?.sender_id === user?.id) && (
              <button onClick={handleDeleteForEveryoneMultiple} className="w-full p-5 text-start text-red-500 flex items-center gap-3 border-t border-slate-700 hover:bg-slate-700 transition-colors"><Trash2 className="w-5 h-5" /> <span className="font-medium">حذف لدى الجميع</span></button>
            )}
          </div>
        </div>
      )}
      ...
      {fullScreenImage && (
        <div className="fixed inset-0 z-[100] bg-black/95 flex flex-col animate-in fade-in duration-200">
          <div className="flex justify-between items-center p-4 bg-black/50 absolute top-0 left-0 right-0 z-10"><button onClick={() => setFullScreenImage(null)} className="text-white p-2 hover:bg-white/20 rounded-full transition-colors"><ArrowLeft className="w-6 h-6" /></button></div>
          <div className="flex-1 flex items-center justify-center overflow-hidden p-2"><img src={fullScreenImage} alt="Fullscreen" className="max-w-full max-h-full object-contain" /></div>
        </div>
      )}
      ...
      <footer className={`px-2 pb-3 mb-safe z-40 w-full relative ${(!contactProfileId && !isLoading) || selectedMessageIds.length > 0 ? 'opacity-40 pointer-events-none' : ''}`}>
        ...
        {replyToMsg && (
          <div className="bg-slate-800/95 backdrop-blur-md border-l-4 border-[#00b4d8] p-3 rounded-t-2xl mb-1 mx-2 flex justify-between items-center shadow-[0_-5px_15px_rgba(0,0,0,0.2)]">
             <div className="flex flex-col overflow-hidden pl-2">
                <span className="text-[#00b4d8] text-[12px] font-bold mb-1">{replyToMsg.sender_id === user?.id ? 'أنت' : contact.name}</span>
                <span className="text-slate-300 text-sm truncate max-w-[250px]">
                  {replyToMsg.content.startsWith('Audio: ') ? '🎵 رسالة صوتية' : replyToMsg.content.startsWith('File: ') ? '📎 ملف مرفق' : replyToMsg.content.replace(/^\[REPLY\|(.*?)\|(.*?)\] /, '')}
                </span>
             </div>
             <button onClick={() => setReplyToMsg(null)} className="p-2 bg-slate-700/50 hover:bg-slate-700 rounded-full text-slate-300 transition-colors"><X className="w-4 h-4" /></button>
          </div>
        )}
        ...
        {recordedAudioBlob ? (
          <div className="flex items-center gap-2 bg-[#009fb7] p-1 pl-2 rounded-[28px] shadow-sm">
            ...
            <div className="flex-1 flex items-center justify-center px-1" dir="ltr" style={{ minWidth: 0 }}><audio src={URL.createObjectURL(recordedAudioBlob)} controls preload="metadata" style={{ display: 'block', width: '100%', minWidth: '200px', maxWidth: '240px', height: '50px', minHeight: '50px', flexShrink: 0 }} className="outline-none" /></div>
          ...
          <div className="flex items-center gap-2 bg-[#009fb7] p-1 pl-2 rounded-[28px] shadow-sm">
            ...
            <div className="flex-1 flex items-center justify-center gap-3 bg-white/10 rounded-full h-[48px]"><div className="w-3 h-3 bg-red-500 rounded-full animate-pulse"></div><span className="text-white font-medium tracking-wide">جاري التسجيل...</span></div>
