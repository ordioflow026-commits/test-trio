    ...
    <div className="w-full h-full bg-slate-100 rounded-[32px] border border-slate-700/50 shadow-2xl overflow-hidden flex flex-col relative pointer-events-auto">
      {canInteract && (
        <div className="absolute top-4 right-6 z-50 flex gap-2">
          <button 
            ...
            className="flex items-center gap-2 px-4 py-2 bg-slate-800 text-white text-sm rounded-full hover:bg-slate-700 shadow-lg transition-colors"
      ...
      <div className="flex-1 w-full h-full p-4 pt-16">
        <textarea
          ...
          placeholder={canInteract ? "اكتب ملاحظاتك المشتركة هنا..." : "في انتظار المضيف..."}
          className="w-full h-full resize-none outline-none bg-transparent text-slate-900 font-medium text-lg custom-scrollbar"
          style={{
            lineHeight: '32px',
            backgroundImage: isLined ? 'repeating-linear-gradient(transparent, transparent 31px, #cbd5e1 31px, #cbd5e1 32px)' : 'none',
