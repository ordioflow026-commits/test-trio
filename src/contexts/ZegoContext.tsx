  ...
  useEffect(() => {
    ...
    const initZego = async () => {
      ...
      const kitToken = ZegoUIKitPrebuilt.generateKitTokenForTest(
        appID, serverSecret, `global_lobby`, safeUserId, userName
      );

      const zp = ZegoUIKitPrebuilt.create(kitToken);

      // 💡 Enhanced Micro-Frontend DOM Injection for Aesthetic & Ultra-Large UI
      zp.addPlugins({
        onIncomingCallReceived: (callID: string, caller: any, callType: number) => {
          const updateTextSafely = () => {
            const containers = document.querySelectorAll('div');
            containers.forEach(div => {
              // Identify Zego's default invitation container
              if (div.innerText && (div.innerText.includes('Incoming video call') || div.innerText.includes('Incoming voice call') || div.innerText.includes('مكالمة فيديو واردة') || div.innerText.includes('مكالمة صوتية واردة'))) {
                
                // Read current language context safely from storage
                const savedLang = localStorage.getItem('language') || 'ar';
                const isAr = savedLang === 'ar';
                
                const isVideo = callType === 1;
                
                // Dynamic Translations without trailing dots
                let text = isVideo 
                ...
                // Injecting an ultra-bold, oversized, and glowing premium user interface wrapper
                div.innerHTML = `
          ...
          // Multistage interval to lock the clean layout overriding native render
          updateTextSafely();
